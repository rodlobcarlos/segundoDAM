...
public class Hotel_repository {
	...
	private Document fromHotelDocumentoJava(Hotel h) {
		...
		for(Habitacion habitacion: h.getHabitaciones()) {
			Document docHabitacion = new Document("tipo", habitacion.getTipo())
					.append("precio", habitacion.getPrecio()).append("capacidad", habitacion.getCapacidad())
					.append("disponible", habitacion.isDiponible());
